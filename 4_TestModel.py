...
from constants_proj.AI_proj_params import PredictionParams, ProjTrainingParams, PreprocParams, NetworkTypes
...
def getNetworkType(name):
    if "unet_multistream" in name:
        return AiModels.UNET_2D_MULTISTREAMSi, NetworkTypes.UNET
    if "unet" in name:
        return AiModels.UNET_2D_SINGLE, NetworkTypes.UNET
    if "simple_cnnverylarge" in name:
        return False, NetworkTypes.SimpleCNN_8
    if "simple_cnn" in name:
        return False, NetworkTypes.SimpleCNN_2

def getInputFields(name):
    if "Input_All_with_Obs_No_SSH_NO_LATLON".lower() in name:
        return ['temp', 'salin', 'u-vel.', 'v-vel.'], ['sst', 'sss']
    if "Temp_with_Obs_No_SSH_NO_LATLON".lower() in name:
        return ['temp', 'salin', 'u-vel.', 'v-vel.'], ['sst', 'sss', 'ssh']
    if "all_input_with_latlon" in name:
        return ['temp', 'salin', 'srfhgt', 'u-vel.', 'v-vel.', 'LAT', 'LON'], ['sst', 'sss', 'ssh']
    if "all_input" in name or "input_all" in name:
        return ['temp', 'salin', 'srfhgt', 'u-vel.', 'v-vel.'], ['sst', 'sss', 'ssh']
    if "input_temp" in name or "onlytemp":
        return ['temp'], []
    return "Unknown"
...
def getOutputFields(name):
    ...
    if "all_output" in name or "output_all" in name:
        return ['temp', 'salin', 'srfhgt', 'u-vel.', 'v-vel.']
    return "Unknown"
...
def main():
    ...
    for model_id in range(len(df)):
        ...
        name = model["Name"]
        network_arch, network_type = getNetworkType(name.lower())
        config[ModelParams.MODEL] = network_arch
        config[ProjTrainingParams.network_type] = network_type
        # Setting input vars
        model_fields, obs_fields = getInputFields(name.lower())
        print(F"Input fields: {model_fields}, {obs_fields}")
        ...
        config[ModelParams.INPUT_SIZE][2] = len(model_fields) + len(obs_fields)
        # Setting output vars
        output_fields = getOutputFields(name.lower())
...
def test_model(config):
    ...
    if net_type == NetworkTypes.SimpleCNN_2:
        # model = simpleCNN(config, nn_type="2d", hid_lay=1, out_lay=1)
        model = simpleSimplest(config)
